...
def intcode_computer(intcode, input1, input2):
	...
	while opCode != '99':
		...
		if opCode == '10':
			...
			if first:
				#print("first: " + str(input1))
				first = False
				val = str(input1)
			elif second:
				#print("second: " + str(input2))
				#second = False
				val = str(input2)
			else:
				#print("val: " + str(outputVal))
				#val = input("test: ")
				val = outputVal
			...
			outputVal = get_num(intcode, opCodePos + 1, int(opCodeFull[2]) or 0)
			opCodePos += 2
...
lastOutput = 0
for i in [4,3,2,1,0]:
	print('lastOutput: ' + str(lastOutput))
	lastOutput = intcode_computer(code, i + 5, lastOutput)
	if not first:
		asdf = asdf
	first = False
